...
Feature: SSM automation document for restore from backup.
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation document to restore from backup
    Given the cloud formation templates as integration test resources
      | CfnTemplatePath                                                                             | ResourceType | DBInstanceClass | AllocatedStorage |
      | resource_manager/cloud_formation_templates/RdsCfnTemplateSingleAz.yml                       | ON_DEMAND    | db.t3.small     | 20               |
    ...
    When SSM automation document "Digito-RdsRestoreFromBackup_2020-04-01" executed
      ...
      | {{cfn-output:RdsCfnTemplateSingleAz>InstanceId}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoRdsRestoreFromBackupAssumeRole}} | false  |
    ...
    And assert db instance {{cfn-output:RdsCfnTemplateSingleAz>InstanceId}} creation date is after {{cache:SsmStepExecutionInterval>1>RecordStartTime>StartTime}}
