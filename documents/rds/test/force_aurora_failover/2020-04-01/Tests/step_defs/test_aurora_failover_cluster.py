...
def cache_db_cluster_instances_before_ssm(boto3_session, resource_manager, ssm_test_cache, reader_cache_key, writer_cache_key, step_key, input_parameters):
    populate_test_cache(boto3_session, resource_manager, ssm_test_cache, reader_cache_key, writer_cache_key, step_key, input_parameters)


@when(parsers.parse('cache DB cluster "{reader_cache_key}" and "{writer_cache_key}" "{step_key}" SSM automation execution\n{input_parameters}'))
def cache_db_cluster_instances_after_ssm(boto3_session, resource_manager, ssm_test_cache, reader_cache_key, writer_cache_key, step_key, input_parameters):
    populate_test_cache(boto3_session, resource_manager, ssm_test_cache, reader_cache_key, writer_cache_key, step_key, input_parameters)
