...
Feature: Alarm Setup - redshift Cluster_Num_exceeded_schema_quotas
  Scenario: Create redshift:alarm:num_exceeded_schema_quotas:2020-04-01 based on NumExceededSchemaQuotas metric and check OK status
    ...
    When alarm "redshift:alarm:num_exceeded_schema_quotas:2020-04-01" is installed
      | alarmId    | ClusterName                                 | Threshold | EvaluationPeriods | DatapointsToAlarm | SNSTopicARN                       |
      | under_test | {{cfn-output:RedshiftTemplate>ClusterName}} | 0         | 1                 | 1                 | {{cfn-output:SnsForAlarms>Topic}} |
