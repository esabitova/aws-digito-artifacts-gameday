...
Feature: SSM automation document for changing execution time limit of Lambda.
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation document for changing execution time limit of Lambda
    ...
    And SSM automation document "Digito-ChangeExecutionTimeLimit_2020-10-26" executed
      ...
      | {{cache:TimeoutValue}} | {{cfn-output:LambdaTemplate>LambdaARN}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoLambdaChangeExecutionTimeLimitAssumeRole}} |
    ...
    And SSM automation document "Digito-ChangeExecutionTimeLimit_2020-10-26" executed
      | NewTimeoutValueSeconds   | LambdaARN                               | AutomationAssumeRole                                                                       |
      | {{cache:before>Timeout}} | {{cfn-output:LambdaTemplate>LambdaARN}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoLambdaChangeExecutionTimeLimitAssumeRole}} |
    And SSM automation document "Digito-ChangeExecutionTimeLimit_2020-10-26" execution in status "Success"
      | ExecutionId                |
