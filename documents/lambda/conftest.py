...
def __populate_cache_with_memory_size(
        resource_manager, ssm_test_cache, cache_property, step_key, param_key, input_parameters
):
    lambda_arn = extract_param_value(
        input_parameters, param_key, resource_manager, ssm_test_cache
    )
    memory_size = lambda_utils.get_memory_size(lambda_arn)
