...
Feature: Alarm Setup - TotalExecutionTime
  ...
  Scenario: Create athena:alarm:total_execution_time_succeeded:2020-04-01 based on TotalExecutionTime metric and check OK status
    ...
    When alarm "athena:alarm:total_execution_time_succeeded:2020-04-01" is installed
    ...
    And execute DML query
      | Database                                       | InputBucketName                                   | AthenaWorkGroupName                               |
      | {{cfn-output:AthenaTemplate>GlueDataBaseName}} | {{cfn-output:AthenaTemplate>S3CrawlerBucketName}} | {{cfn-output:AthenaTemplate>AthenaWorkGroupName}} |
    Then assert metrics for all alarms are populated
    And Wait until alarm {{alarm:under_test>AlarmName}} becomes OK within 180 seconds, check every 15 seconds

  Scenario: Create athena:alarm:total_execution_time_succeeded:2020-04-01 based on TotalExecutionTime metric and check ALARM status
    Given the cloud formation templates as integration test resources
      | CfnTemplatePath                                                             | ResourceType | CleanupS3BucketLambdaArn                                      | KmsKey                              |
      | resource_manager/cloud_formation_templates/shared/CleanupS3BucketLambda.yml | SHARED       |                                                               |                                     |
      | resource_manager/cloud_formation_templates/shared/KMS.yml                   | SHARED       |                                                               |                                     |
