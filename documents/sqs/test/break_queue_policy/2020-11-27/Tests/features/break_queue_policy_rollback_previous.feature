...
Feature: SSM automation document to to test behavior when messages cannot be sent to an SQS queue
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation document to test behavior when messages cannot be sent to an SQS queue
    ...
    # Terminate execution
    When Wait for the SSM automation document "Digito-BreakSQSQueuePolicyTest_2020-11-27" execution is on step "UpdateQueuePolicy" in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    Then terminate "Digito-BreakSQSQueuePolicyTest_2020-11-27" SSM automation document
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    ...
    And Wait for the SSM automation document "Digito-BreakSQSQueuePolicyTest_2020-11-27" execution is on step "TriggerRollback" in status "Success"
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
    And SSM automation document "Digito-BreakSQSQueuePolicyTest_2020-11-27" execution in status "Cancelled"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    ...
    Then cache rollback execution id
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
    When SSM automation document "Digito-BreakSQSQueuePolicyTest_2020-11-27" execution in status "Success"
      | ExecutionId                |
