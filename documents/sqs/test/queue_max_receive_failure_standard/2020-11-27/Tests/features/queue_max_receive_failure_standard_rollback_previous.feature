...
Feature: SSM automation document to test behavior of Standard Queue after receiving a message maximum allowed times
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation document to test rollback of previous execution
    Given the cloud formation templates as integration test resources
      ...
      | documents/sqs/test/queue_max_receive_failure_standard/2020-11-27/Documents/AutomationAssumeRoleTemplate.yml | ASSUME_ROLE  |
    And published "Digito-ForceSQSStandardQueueMaxReceiveFailureTest_2020-11-27" SSM document
    ...
    And cache number of messages in queue as "NumberOfMessagesDLQ" "before" SSM automation execution
      | QueueUrl                                             |
      | {{cfn-output:SqsTemplate>SqsDlqForStandardQueueUrl}} |
    And SSM automation document "Digito-ForceSQSStandardQueueMaxReceiveFailureTest_2020-11-27" executed
      | QueueUrl                                       | AutomationAssumeRole                                                                                   | DeadLetterQueueAlarmName                                |
      | {{cfn-output:SqsTemplate>SqsStandardQueueUrl}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoForceSQSStandardQueueMaxReceiveFailureTestAssumeRole}} | {{cfn-output:SqsTemplate>DlqMessageStandardQueueAlarm}} |
    And send "100" messages to queue
      | QueueUrl                                       |
      | {{cfn-output:SqsTemplate>SqsStandardQueueUrl}} |

    When Wait for the SSM automation document "Digito-ForceSQSStandardQueueMaxReceiveFailureTest_2020-11-27" execution is on step "AssertAlarmToBeRed" in status "InProgress"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    Then terminate "Digito-ForceSQSStandardQueueMaxReceiveFailureTest_2020-11-27" SSM automation document
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    Then Wait for the SSM automation document "Digito-ForceSQSStandardQueueMaxReceiveFailureTest_2020-11-27" execution is on step "TriggerRollback" in status "Success"
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
    And SSM automation document "Digito-ForceSQSStandardQueueMaxReceiveFailureTest_2020-11-27" execution in status "Cancelled"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    ...
    When SSM automation document "Digito-ForceSQSStandardQueueMaxReceiveFailureTest_2020-11-27" execution in status "Success"
      | ExecutionId                |
