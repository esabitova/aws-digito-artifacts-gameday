...
Feature: SSM automation document to test behavior of FIFO queue after receiving a message maximum allowed times
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation document to test rollback of a previous execution
    ...
    And SSM automation document "Digito-ForceSQSFifoQueueMaxReceiveFailureTest_2020-11-27" executed
      ...
      | {{cfn-output:SqsTemplate>SqsFifoQueueEnabledDlqUrl}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoForceSQSFifoQueueMaxReceiveFailureTestAssumeRole}} | {{cfn-output:SqsTemplate>DlqMessageFifoQueueAlarm}} |
    And send "100" messages to FIFO queue
      | QueueUrl                                             |
      | {{cfn-output:SqsTemplate>SqsFifoQueueEnabledDlqUrl}} |

    When Wait for the SSM automation document "Digito-ForceSQSFifoQueueMaxReceiveFailureTest_2020-11-27" execution is on step "AssertAlarmToBeRed" in status "InProgress"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    Then terminate "Digito-ForceSQSFifoQueueMaxReceiveFailureTest_2020-11-27" SSM automation document
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    Then Wait for the SSM automation document "Digito-ForceSQSFifoQueueMaxReceiveFailureTest_2020-11-27" execution is on step "TriggerRollback" in status "Success"
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
    And SSM automation document "Digito-ForceSQSFifoQueueMaxReceiveFailureTest_2020-11-27" execution in status "Cancelled"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
