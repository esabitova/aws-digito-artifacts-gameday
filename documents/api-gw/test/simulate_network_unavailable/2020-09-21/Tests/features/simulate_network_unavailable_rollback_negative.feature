...
Feature: SSM automation document Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21
  ...
  Scenario: Execute SSM automation document Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21 to test rollback failure when inputs different than original execution
    ...
    When SSM automation document "Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21" executed
      ...
      | {{cfn-output:RestApiGwPrivateEndpointTemplate>RestApiGwId}} | {{cfn-output:RestApiGwPrivateEndpointTemplate>SecurityGroupId}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoSimulateRestApiGwNetworkUnavailableTestAssumeRole}} | {{cfn-output:RestApiGwPrivateEndpointTemplate>CountAlarmName}} |

    Then Wait for the SSM automation document "Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21" execution is on step "InjectFailure" in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And terminate "Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21" SSM automation document
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And Wait for the SSM automation document "Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21" execution is on step "TriggerRollback" in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    ...
    Then SSM automation document "Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21" executed
      | RestApiGwId      | IsRollback | PreviousExecutionId        | AutomationAssumeRole                                                                                | ApiGwCountAlarmName                                            |
      | WrongRestApiGwId | True       | {{cache:SsmExecutionId>1}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoSimulateRestApiGwNetworkUnavailableTestAssumeRole}} | {{cfn-output:RestApiGwPrivateEndpointTemplate>CountAlarmName}} |
    And SSM automation document "Digito-SimulateRestApiGwNetworkUnavailableTest_2020-09-21" execution in status "Success"
      | ExecutionId                |
