...
Feature: SSM automation document Digito-ExceedRestApiGwQuotaTest_2020-09-21
  ...
  Scenario: Execute SSM automation document Digito-ExceedRestApiGwQuotaTest_2020-09-21 in negative rollback test
    ...
    Then Wait for the SSM automation document "Digito-ExceedRestApiGwQuotaTest_2020-09-21" execution is on step "BackupQuotaConfiguration" in status "Success"
      | ExecutionId                |
    ...
    Then cache rollback execution id
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And Wait for the SSM automation document "Digito-ExceedRestApiGwQuotaTest_2020-09-21" execution is on step "RollbackPreviousExecution" in status "Success"
      | ExecutionId                |
    ...
    Then SSM automation document "Digito-ExceedRestApiGwQuotaTest_2020-09-21" executed
      | RestApiGwUsagePlanId | IsRollback | PreviousExecutionId        | AutomationAssumeRole                                                                 | ApiGw4xxAlarmName                                  |
      | WrongUsagePlanID     | True       | {{cache:SsmExecutionId>1}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoExceedRestApiGwQuotaTestAssumeRole}} | {{cfn-output:RestApiGwTemplate>4XXErrorAlarmName}} |
    And SSM automation document "Digito-ExceedRestApiGwQuotaTest_2020-09-21" execution in status "Success"
      | ExecutionId                |
