...
test: test_linter
	...
	export AWS_PROFILE=${AWS_PROFILE}; python3 -m pytest --count=${TEST_COUNT} --workers ${TEST_WORKERS} --pool_size ${TEST_POOL_SIZE} \
		${TEST_TARGETS} --keep_test_resources --release_failed_resources --run_integration_tests -m ${SERVICE} --aws_profile ${AWS_PROFILE} && \
	deactivate
...
service_unit_test:
	cd ../../../ && \
	source venv/bin/activate && \
	python3 -m pytest -m unit_test --no-cov documents/util/scripts/test/test_apigw_utils.py && \
	deactivate
