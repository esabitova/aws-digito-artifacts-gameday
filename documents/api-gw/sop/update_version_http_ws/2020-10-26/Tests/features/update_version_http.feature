...
Feature: SSM automation document to update api gateway deployment version
  ...
  Scenario: Run document for HTTP Api with a specified deployment Id
    ...
    When SSM automation document "Digito-UpdateVersionHttpWs_2020-10-26" executed
      ...
      | {{cfn-output:HTTPWSApiGwTemplate>HttpApiGwId}} | {{cfn-output:HTTPWSApiGwTemplate>HttpStageName}} | {{cache:before>DeploymentIdToApply}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoApiGwUpdateVersionHttpWsAssumeRole}} |
  ...
  Scenario: Run document for WS Api for stage with AutoDeploy and expect failure as AutoDeploy is not supported
    ...
    When SSM automation document "Digito-UpdateVersionHttpWs_2020-10-26" executed
      | HttpWsApiGwId                                  | HttpWsStageName                                            | HttpWsDeploymentId                   | AutomationAssumeRole                                                                 |
      | {{cfn-output:HTTPWSApiGwTemplate>HttpApiGwId}} | {{cfn-output:HTTPWSApiGwTemplate>HttpStageNameAutoDeploy}} | {{cache:before>DeploymentIdToApply}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoApiGwUpdateVersionHttpWsAssumeRole}} |
  ...
  Scenario: Run document for HTTP Api with provided deployment same as current
    ...
    When SSM automation document "Digito-UpdateVersionHttpWs_2020-10-26" executed
      | HttpWsApiGwId                                  | HttpWsStageName                                  | HttpWsDeploymentId            | AutomationAssumeRole                                                                 |
      | {{cfn-output:HTTPWSApiGwTemplate>HttpApiGwId}} | {{cfn-output:HTTPWSApiGwTemplate>HttpStageName}} | {{cache:before>DeploymentId}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoApiGwUpdateVersionHttpWsAssumeRole}} |
    Then SSM automation document "Digito-UpdateVersionHttpWs_2020-10-26" execution in status "Failed"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
  ...
  Scenario: Run document for HTTP Api without provided deployment Id
    ...
    When SSM automation document "Digito-UpdateVersionHttpWs_2020-10-26" executed
      ...
      | {{cfn-output:HTTPWSApiGwTemplate>HttpApiGwId}} | {{cfn-output:HTTPWSApiGwTemplate>HttpStageName}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoApiGwUpdateVersionHttpWsAssumeRole}} |
