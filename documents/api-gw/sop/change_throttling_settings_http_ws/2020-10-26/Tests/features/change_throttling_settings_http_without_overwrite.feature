...
Feature: SSM automation document to change HTTP or WS API GW route throttling settings
  ...
  Scenario: Change throttling settings for HTTP API Gateway with throttling enabled, ForceExecution=False and with the specified route key
    ...
    When SSM automation document "Digito-ChangeThrottlingSettingsHttpWs_2020-10-26" executed
      | HttpWsApiGwId                                  | HttpWsStageName                                           | HttpWsThrottlingRate               | HttpWsThrottlingBurst               | HttpWsRouteKey | AutomationAssumeRole                                                                            |
      | {{cfn-output:HTTPWSApiGwTemplate>HttpApiGwId}} | {{cfn-output:HTTPWSApiGwTemplate>HttpStageNameThrottled}} | {{cache:before>ExpectedRateLimit}} | {{cache:before>ExpectedBurstLimit}} | POST /test     | {{cfn-output:AutomationAssumeRoleTemplate>DigitoApiGwChangeThrottlingSettingsHttpWsAssumeRole}} |
    Then SSM automation document "Digito-ChangeThrottlingSettingsHttpWs_2020-10-26" execution in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And cache route throttling rate limit as "RateLimit" and burst limit as "BurstLimit" "after" SSM automation execution
  ...
  Scenario: Fail to change rate limit above account quota for HTTP API Gateway with throttling enabled, ForceExecution=False and with the specified route key
    ...
    When SSM automation document "Digito-ChangeThrottlingSettingsHttpWs_2020-10-26" executed
      | HttpWsApiGwId                                  | HttpWsStageName                                           | HttpWsThrottlingRate | HttpWsThrottlingBurst               | HttpWsRouteKey | AutomationAssumeRole                                                                            |
      | {{cfn-output:HTTPWSApiGwTemplate>HttpApiGwId}} | {{cfn-output:HTTPWSApiGwTemplate>HttpStageNameThrottled}} | 20000                | {{cache:before>ExpectedBurstLimit}} | POST /test     | {{cfn-output:AutomationAssumeRoleTemplate>DigitoApiGwChangeThrottlingSettingsHttpWsAssumeRole}} |
    Then SSM automation document "Digito-ChangeThrottlingSettingsHttpWs_2020-10-26" execution in status "Failed"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And cache route throttling rate limit as "RateLimit" and burst limit as "BurstLimit" "after" SSM automation execution
  ...
  Scenario: Fail to change burst limit above account quota for HTTP API Gateway with throttling enabled, ForceExecution=False and with the specified route key
    ...
    When SSM automation document "Digito-ChangeThrottlingSettingsHttpWs_2020-10-26" executed
      | HttpWsApiGwId                                  | HttpWsStageName                                           | HttpWsThrottlingRate               | HttpWsThrottlingBurst | HttpWsRouteKey | AutomationAssumeRole                                                                            |
      | {{cfn-output:HTTPWSApiGwTemplate>HttpApiGwId}} | {{cfn-output:HTTPWSApiGwTemplate>HttpStageNameThrottled}} | {{cache:before>ExpectedRateLimit}} | 10000                 | POST /test     | {{cfn-output:AutomationAssumeRoleTemplate>DigitoApiGwChangeThrottlingSettingsHttpWsAssumeRole}} |
    Then SSM automation document "Digito-ChangeThrottlingSettingsHttpWs_2020-10-26" execution in status "Failed"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And cache route throttling rate limit as "RateLimit" and burst limit as "BurstLimit" "after" SSM automation execution
