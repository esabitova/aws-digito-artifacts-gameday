...
Feature: Alarm Setup - API Gateway Errors
  Scenario: Test API Gateway api-gw:alarm:health-5xx_error:2020-04-01
    ...
    When alarm "api-gw:alarm:health-5xx_error:2020-04-01" is installed
      |alarmId    | ApiName                                  | Threshold | SNSTopicARN                      |
      |under_test | {{cfn-output:RestApiGwTemplate>ApiName}} | 0         | {{cfn-output:SnsForAlarms>Topic}}|
    And get API key and perform "120" https "GET" requests with interval "2" seconds
