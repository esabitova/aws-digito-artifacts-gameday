...
Feature: Alarm Setup - load-balancer UnHealthyHostCount
  ...
  Scenario: Create load-balancer:alarm:gateway_unhealthy_host_count:2020-04-01 based on UnHealthyHostCount metric and check OK status
    ...
    When alarm "load-balancer:alarm:gateway_unhealthy_host_count:2020-04-01" is installed
  ...
  Scenario: Create load-balancer:alarm:gateway_unhealthy_host_count:2020-04-01 based on UnHealthyHostCount metric and check ALARM status
    Given the cloud formation templates as integration test resources
      | CfnTemplatePath                                                            | ResourceType | VPC                      | Subnet                                             | EC2Subnet                                      | VPCCidr                    |
      | resource_manager/cloud_formation_templates/shared/VPC.yml                  | SHARED       |                          |                                                    |                                                |                            |
      | resource_manager/cloud_formation_templates/GatewayLoadBalancerTemplate.yml | ON_DEMAND    | {{cfn-output:VPC>VPCId}} | {{cfn-output:VPC>PrivateSubnetWithoutInternetOne}} | {{cfn-output:VPC>PrivateSubnetWithInternet01}} | {{cfn-output:VPC>VPCCidr}} |
