...
Feature: SSM automation document Digito-ForceNetworkGwLbTargetUnavailableTest_2020-04-01
  Scenario: Create Network LB and execute automation to make the target group unavailable in rollback
    ...
    And Wait for the SSM automation document "Digito-ForceNetworkGwLbTargetUnavailableTest_2020-04-01" execution is on step "AssertAlarmToBeRed" in status "InProgress" for "600" seconds
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And cache target group HealthCheckPort as "NewPort" "after" SSM automation execution
      | LoadBalancerArn                                           |
      | {{cfn-output:NetworkLoadBalancerTemplate>NetworkELBArn}}  |
    And terminate "Digito-ForceNetworkGwLbTargetUnavailableTest_2020-04-01" SSM automation document
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |

    Then Wait for the SSM automation document "Digito-ForceNetworkGwLbTargetUnavailableTest_2020-04-01" execution is on step "TriggerRollback" in status "Success" for "240" seconds
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And SSM automation document "Digito-ForceNetworkGwLbTargetUnavailableTest_2020-04-01" execution in status "Cancelled"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And cache rollback execution id
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And SSM automation document "Digito-ForceNetworkGwLbTargetUnavailableTest_2020-04-01" execution in status "Success"
      | ExecutionId                |
