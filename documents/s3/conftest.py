...
@given(parsers.parse('put "{object_key_to_put}" object "{times_to_put}" times with different content '
                     ...
                     '\n{input_parameters}'))
def put_different_objects_to_bucket(boto3_session, resource_manager, ssm_test_cache, object_key_to_put,
                                    times_to_put, s3_bucket_name_property, input_parameters):
    s3_bucket_name: str = extract_param_value(input_parameters, s3_bucket_name_property, resource_manager,
                                              ssm_test_cache)
...
@given(parsers.parse('cache value of "{s3_property_name}" property of the "{object_key}" file '
                     ...
                     'at "{s3_bucket_param_key}" bucket "{step_key}" SSM automation execution'
                     '\n{input_parameters}'))
def cache_property_of_version(boto3_session, resource_manager, ssm_test_cache, s3_property_name, object_key,
                              version_id_param_key, cache_property, s3_bucket_param_key, step_key, input_parameters):
    populate_cache_property_of_version(boto3_session, resource_manager, ssm_test_cache, s3_property_name, object_key,
...
def populate_cache_property_of_version(boto3_session: Session, resource_manager, ssm_test_cache, s3_property_name,
                                       object_key, version_id_param_key, cache_property, s3_bucket_param_key, step_key,
                                       input_parameters):
    s3_bucket_name: str = extract_param_value(input_parameters, s3_bucket_param_key, resource_manager, ssm_test_cache)
    version_id: str = extract_param_value(input_parameters, version_id_param_key, resource_manager, ssm_test_cache)
...
def put_objects_to_bucket(boto3_session, resource_manager, ssm_test_cache, number_of_files_to_put, input_parameters):
...
def cache_value_before_ssm(boto3_session, resource_manager, ssm_test_cache, cache_property, step_key, input_parameters):
    populate_cache_of_number_of_files(boto3_session, resource_manager, ssm_test_cache, cache_property, step_key,
                                      "BucketName", input_parameters)


def populate_cache_of_number_of_files(boto3_session: Session, resource_manager, ssm_test_cache, cache_property,
