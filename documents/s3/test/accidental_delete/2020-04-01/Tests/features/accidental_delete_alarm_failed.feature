...
Feature: SSM automation document to accidentally delete files in S3 bucket
  ...
  Scenario: Execute Digito-SimulateS3ObjectsAccidentalDeleteTest_2020-04-01 to accidentally delete files in S3 bucket and fail because of timed out alarm instead of being in ALARM state
...
#    If we don't trigger the alarm, step has to fail and cause rollback, step and ssm will have TimedOut state
    And Wait for the SSM automation document "Digito-SimulateS3ObjectsAccidentalDeleteTest_2020-04-01" execution is on step "AssertAlarmToBeRed" in status "TimedOut"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    ...
    And Wait for the SSM automation document "Digito-SimulateS3ObjectsAccidentalDeleteTest_2020-04-01" execution is on step "AssertAlarmToBeGreen" in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And SSM automation document "Digito-SimulateS3ObjectsAccidentalDeleteTest_2020-04-01" execution in status "TimedOut"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
