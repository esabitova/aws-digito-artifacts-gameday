...
#todo copy from aws-documents repo    And assert "NumberOfFilesAfterDeletion" at "before" became not equal to "0"
    And get the "object-to-restore-versions.txt" object from bucket "20" times with error
      | BucketName                                      |
      | {{cfn-output:S3Template>S3BucketToRestoreName}} |
    When SSM automation document "Digito-AccidentalDeleteS3Objects_2020-04-01" execution in status "Success"
      | ExecutionId                |
