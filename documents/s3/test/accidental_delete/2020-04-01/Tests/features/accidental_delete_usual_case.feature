...
Feature: SSM automation document to accidentally delete files in S3 bucket
  ...
  Scenario: Execute Digito-AccidentalDeleteS3Objects_2020-04-01 to to accidentally delete files in S3 bucket
    ...
    And get the "0.txt" object from bucket "20" times with error
      | BucketName                                      |
      | {{cfn-output:S3Template>S3BucketToRestoreName}} |
    And Wait for the SSM automation document "Digito-AccidentalDeleteS3Objects_2020-04-01" execution is on step "RollbackCurrentExecution" in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And get the "0.txt" object from bucket "20" times
      | BucketName                                      |
      | {{cfn-output:S3Template>S3BucketToRestoreName}} |
    And SSM automation document "Digito-AccidentalDeleteS3Objects_2020-04-01" execution in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
