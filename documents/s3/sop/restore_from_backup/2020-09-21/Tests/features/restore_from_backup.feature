...
Feature: SSM automation document to restore an S3 bucket from a backup bucket
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation document to restore an S3 bucket from a backup bucket without approval to clean the restore bucket
    ...
    And SSM automation document "Digito-RestoreS3BucketFromBackupSOP_2020-09-21" executed
      ...
      | {{cfn-output:S3Template>S3BackupBucketName}} | {{cfn-output:S3Template>S3BucketToRestoreName}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoRestoreS3BucketFromBackupSOPAssumeRole}} | {{cfn-output:S3Template>SNSTopicName}} | {{cfn-output:S3Template>RoleApproveCleanBucketArn}} | true                                   |


    When SSM automation document "Digito-RestoreS3BucketFromBackupSOP_2020-09-21" execution in status "Success"
      | ExecutionId                |
