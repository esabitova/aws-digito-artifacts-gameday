...
Feature: SSM automation document EC2 memory stress testing
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation memory stress on EC2 instance
    Given the cached input parameters
      |AlarmGreaterThanOrEqualToThreshold|InstanceType|MemoryLoadPercentage|StressDuration|AlarmNamespace|
      |                                50|    t2.small|                  90|           300|       CWAgent|
    And the cloud formation templates as integration test resources
      ...
      |documents/stateless-compute/test/ec2-inject_memory_load/2020-07-28/Documents/AutomationAssumeRoleTemplate.yml | ASSUME_ROLE|                      |                                            |
    ...
    Then assert "mem_used_percent" metric reached "60" percent(s)
      |ExecutionId               |StepName       | InstanceId                                        |ImageId                                         |InstanceType          |Namespace               |
      |{{cache:SsmExecutionId>1}}|RunMemoryStress|{{cfn-output:EC2WithCWAgentCfnTemplate>InstanceId}}|{{cfn-output:EC2WithCWAgentCfnTemplate>ImageId}}|{{cache:InstanceType}}|{{cache:AlarmNamespace}}|
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation memory stress on EC2 instance with rollback
    ...
    When assert "mem_used_percent" metric reached "60" percent(s)
      |ExecutionId               |StepName       | InstanceId                                        |ImageId                                         |InstanceType          |Namespace               |
      |{{cache:SsmExecutionId>1}}|RunMemoryStress|{{cfn-output:EC2WithCWAgentCfnTemplate>InstanceId}}|{{cfn-output:EC2WithCWAgentCfnTemplate>ImageId}}|{{cache:InstanceType}}|{{cache:AlarmNamespace}}|
    ...
    # https://issues.amazon.com/issues/Digito-1742
    Then sleep for "120" seconds
    And assert "mem_used_percent" metric below "10" percent(s) after rollback
