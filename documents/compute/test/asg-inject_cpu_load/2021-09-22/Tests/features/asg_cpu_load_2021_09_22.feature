@asg @cpu @stress @integration @2021_09_22
Feature: SSM automation document ASG CPU stress testing
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation CPU stress on ASG instances
    Given the cached input parameters
      |InstanceType|LoadPercent|DurationSeconds|AlarmNamespace|MetricPeriod|PercentageOfInstances|ExpectedRecoveryTime|
    ...
    And the cloud formation templates as integration test resources
      ...
      |documents/compute/test/asg-inject_cpu_load/2021-09-22/Documents/AutomationAssumeRoleTemplate.yml| ASSUME_ROLE|                      |
    And published "Digito-SimulateHighCpuLoadInAsg_2021-09-22" SSM document

    When SSM automation document "Digito-SimulateHighCpuLoadInAsg_2021-09-22" executed
      |AutoScalingGroupName                               |AutomationAssumeRole                                                                |CpuUtilizationAlarmName                                 |LoadPercent          |DurationSeconds          |PercentageOfInstances          |ExpectedRecoveryTime          |
      |{{cfn-output:AsgCfnTemplate>AutoScalingGroupName}} |{{cfn-output:AutomationAssumeRoleTemplate>DigitoSimulateHighCpuLoadInAsgAssumeRole}}|{{cfn-output:AsgCfnTemplate>AsgCpuUtilizationAlarmName}}|{{cache:LoadPercent}}|{{cache:DurationSeconds}}|{{cache:PercentageOfInstances}}|{{cache:ExpectedRecoveryTime}}|
    And SSM automation document "Digito-SimulateHighCpuLoadInAsg_2021-09-22" execution in status "Success"
      |ExecutionId               |
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation CPU stress on ASG instances with rollback
    ...
    Then Wait for the SSM automation document "Digito-SimulateHighCpuLoadInAsg_2021-09-22" execution is on step "RunCpuStress" in status "InProgress" for "600" seconds
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
    ...
    And terminate "Digito-SimulateHighCpuLoadInAsg_2021-09-22" SSM automation document
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|

    Then Wait for the SSM automation document "Digito-SimulateHighCpuLoadInAsg_2021-09-22" execution is on step "TriggerRollback" in status "Success" for "240" seconds
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|

    Then SSM automation document "Digito-SimulateHighCpuLoadInAsg_2021-09-22" execution in status "Cancelled"
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
