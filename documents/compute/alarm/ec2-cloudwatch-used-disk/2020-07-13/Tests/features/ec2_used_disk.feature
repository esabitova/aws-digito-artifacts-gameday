@ec2 @integration @alarm @disk
Feature: Alarm Setup - EC2 Used Disk
  Scenario: Lease EC2 from resource manager and test attach an alarm from Document
    ...
    When alarm "compute:alarm:ec2-cloudwatch-used-disk:2020-07-13" is installed
      |alarmId    |InstanceId                                          |Threshold  | SNSTopicARN
      |under_test |{{cfn-output:EC2WithCWAgentCfnTemplate>InstanceId}} |   95      | {{cfn-output:SnsForAlarms>Topic}}
