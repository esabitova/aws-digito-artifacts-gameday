...
Feature: SSM automation document to restore the database from point in time.
  ...
  Scenario: Restores table to the specific point
    ...
    When SSM automation document "Digito-RestoreDynamoDBTableToPointInTimeSOP_2020-04-01" executed
      ...
      | {{cfn-output:DynamoDBTemplate>DynamoDBTable}} | {{cache:TargetTableToRestoreName}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoRestoreDynamoDBTableFromPointInTimeSOPAssumeRole}} | {{cache:ValidRecoveryPoint}} |
