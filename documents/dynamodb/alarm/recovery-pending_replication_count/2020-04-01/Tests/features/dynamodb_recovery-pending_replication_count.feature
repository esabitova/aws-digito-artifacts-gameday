...
Feature: Alarm Setup - dynamodb PendingReplicationCount
  Scenario: pending_replication_count - green
    ...
    And enabled global dynamodb table {{cfn-output:DynamoDBTemplateWithAutoScaling>DynamoDBTable}} in the region {{cache:before>GlobalTableSecondaryRegion}} and wait for 3600 seconds with delay 20 seconds
