...
Feature: Alarm Setup - Amazon DynamoDB SuccessfulRequestLatency BatchWriteItem
  Scenario: Create dynamodb:alarm:health-successful_request_latency_batch_write_item:2020-04-01 based on SuccessfulRequestLatency and check OK status
    ...
    When alarm "dynamodb:alarm:health-successful_request_latency_batch_write_item:2020-04-01" is installed
  ...
  Scenario: Create dynamodb:alarm:health-successful_request_latency_batch_write_item:2020-04-01 based on SuccessfulRequestLatency and check ALARM status
    Given the cloud formation templates as integration test resources
      | CfnTemplatePath                                                    | ResourceType
      | resource_manager/cloud_formation_templates/DynamoDBTemplate.yml    | ON_DEMAND
      | resource_manager/cloud_formation_templates/shared/SnsForAlarms.yml | SHARED
