...
Feature: Alarm Setup - Amazon DynamoDB SuccessfulRequestLatency TransactGetItems
  Scenario: Create dynamodb:alarm:health-successful_request_latency_transact_get_items:2020-04-01 based on SuccessfulRequestLatency and check OK status
    ...
    When alarm "dynamodb:alarm:health-successful_request_latency_transact_get_items:2020-04-01" is installed
    ...
    And get items from cache using key "TestDynamoDBItems" and transact get it divided by time from dynamodb table
      | DynamoDBTableName                             |
      | {{cfn-output:DynamoDBTemplate>DynamoDBTable}} |
    Then assert metrics for all alarms are populated
    And wait until alarm {{alarm:under_test>AlarmName}} becomes OK within 1200 seconds, check every 15 seconds

  Scenario: Create dynamodb:alarm:health-successful_request_latency_transact_get_items:2020-04-01 based on SuccessfulRequestLatency and check ALARM status
    Given the cloud formation templates as integration test resources
      | CfnTemplatePath                                                    | ResourceType
      | resource_manager/cloud_formation_templates/DynamoDBTemplate.yml    | ON_DEMAND
      | resource_manager/cloud_formation_templates/shared/SnsForAlarms.yml | SHARED
