...
Feature: Alarm Setup - Backup NumberOfBackupJobsFailed
  Scenario: Attach NumberOfBackupJobsFailed alarm to Document and trigger it
    Given the cloud formation templates as integration test resources
      | CfnTemplatePath                                                    | ResourceType |
      | resource_manager/cloud_formation_templates/BackupTemplate.yml      | ON_DEMAND    |
      | resource_manager/cloud_formation_templates/shared/SnsForAlarms.yml | SHARED       |
    When alarm "backup:alarm:backup-jobs-failed:2020-12-06" is installed
