...
Feature: Alarm Setup - Kinesis Analytics Errors
  Scenario: Create kinesis:alarm:data_millis_behind_latest_records:2020-04-01 based on millisBehindLatest metric and check OK status
    ...
    And start Kinesis Data Analytics for "Apache Flink" application
      ...
      | {{cfn-output:KinesisAnalyticsTemplateFlink>KinesisAnalyticsApplicationPhysicalResourceId}} |
    When alarm "kinesis-analytics:alarm:health-millisbehindlatest-flink:2020-04-01" is installed
      | alarmId    | KinesisAnalyticsInputId                  | KinesisAnalyticsAppName                                                                    | Threshold | SNSTopicARN                       | EvaluationPeriods | DatapointsToAlarm |
      | under_test | {{cache:before>KinesisAnalyticsInputId}} | {{cfn-output:KinesisAnalyticsTemplateFlink>KinesisAnalyticsApplicationPhysicalResourceId}} |    5      | {{cfn-output:SnsForAlarms>Topic}} |     1             |     1             |
