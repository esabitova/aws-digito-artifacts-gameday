...
Feature: Alarm Setup - Kinesis Analytics Errors
  Scenario: Create kinesis:alarm:data_analytics_inputprocessing_processingfailedrecords:2020-04-01 based on InputProcessing.ProcessingFailedRecords metric and check OK status
    ...
    When alarm "kinesis-analytics:alarm:health-processing_failed_records:2020-04-01" is installed
      | alarmId    | KinesisAnalyticsInputId                  | KinesisAnalyticsAppName                                                                  | Threshold | SNSTopicARN                       | EvaluationPeriods | DatapointsToAlarm |
      | under_test | {{cache:before>KinesisAnalyticsInputId}} | {{cfn-output:KinesisAnalyticsTemplateSQL>KinesisAnalyticsApplicationPhysicalResourceId}} |    0      | {{cfn-output:SnsForAlarms>Topic}} |     1             |     1             |
