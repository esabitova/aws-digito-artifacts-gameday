...
Feature: SSM automation document Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28
  ...
  Scenario: Execute SSM automation document Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28 in rollback
    ...
    And alarm "kinesis-analytics:alarm:health-downtime:2020-04-01" is installed
      | alarmId    | SNSTopicARN                       | KinesisAnalyticsAppName                                                                        | Threshold  | DatapointsToAlarm | EvaluationPeriods |
      | under_test | {{cfn-output:SnsForAlarms>Topic}} | {{cfn-output:KinesisAnalyticsTemplateFlinkVPC>KinesisAnalyticsApplicationPhysicalResourceId}}} | 0          | 1                 | 1                 |
    ...
    When SSM automation document "Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28" executed
      ...
      | {{cfn-output:KinesisAnalyticsTemplateFlinkVPC>KinesisAnalyticsApplicationPhysicalResourceId}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoKinesisAnalyticsBreakVpcConfigurationAssumeRole}} | {{alarm:under_test>AlarmName}} |
    And Wait for the SSM automation document "Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28" execution is on step "AssertAlarmToBeRed" in status "InProgress"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And terminate "Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28" SSM automation document
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |    
    Then Wait for the SSM automation document "Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28" execution is on step "TriggerRollback" in status "Success"
      | ExecutionId               |
      | {{cache:SsmExecutionId>1}}|
    And SSM automation document "Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28" execution in status "Cancelled"
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
    And cache rollback execution id
      |ExecutionId               |
      |{{cache:SsmExecutionId>1}}|
    And SSM automation document "Digito-BreakKinesisDataAnalyticsVpcConfigurationTest_2021-07-28" execution in status "Success"
      |ExecutionId               |
    ...
    And assert "CheckIsRollback,AssertAlarmToBeGreenBeforeTest,CheckKinesisDataAnalyticsApplicationVPCStatus,AwaitKinesisDataAnalyticsRunningStatusBeforeBackupCurrentExecution,
                BackupCurrentExecution,CreateDummySecurityGroup,InjectFailure,AwaitApplicationUpdateCompleteAfterInjectFailure,
                TriggerRollback" steps are successfully executed in order
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And assert "CheckIsRollback,GetInputsFromPreviousExecution,AssertInputsFromPreviousExecution,AwaitKinesisDataAnalyticsRunningStatusBeforeRollbackOfPreviousExecution,
                PrepareRollbackOfPreviousExecutionGetOriginalSecurityGroup,PrepareRollbackOfPreviousExecutionGetDummySecurityGroup,RollbackPreviousExecution,
                AwaitApplicationIsRunningAfterRollbackPreviousExecution,RemoveDummySecurityGroupAfterRollbackPreviousExecution" steps are successfully executed in order
      | ExecutionId                |
      | {{cache:SsmExecutionId>2}} |
