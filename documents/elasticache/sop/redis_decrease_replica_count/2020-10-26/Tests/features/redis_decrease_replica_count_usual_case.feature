...
Feature: SSM automation document Digito-RedisDecreaseReplicaCount_2020-10-26
  ...
  Scenario: Execute SSM automation document Digito-RedisDecreaseReplicaCount_2020-10-26 to decrease redis replica count with node ids for replicas specified
    ...
    When SSM automation document "Digito-RedisDecreaseReplicaCount_2020-10-26" executed
      ...
      | {{cfn-output:ElasticacheReplicationGroupClusterDisabledSingleAZ>ReplicationGroupId}}  | {{cache:before>NodeID}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoElasticacheRedisDecreaseReplicaCountAssumeRole}} |
