...
Feature: Alarm Setup - Elasticache Errors
  Scenario: Create elasticache:alarm:health-master_link_health_status:2020-04-01 based on MasterLinkHealthStatus and check OK status
    ...
    When alarm "elasticache:alarm:health-master_link_health_status:2020-04-01" is installed
      | alarmId    | ClusterId                         | Threshold | SNSTopicARN                       |
      | under_test | {{cache:before>ReplicaClusterId}} | 1         | {{cfn-output:SnsForAlarms>Topic}} |
