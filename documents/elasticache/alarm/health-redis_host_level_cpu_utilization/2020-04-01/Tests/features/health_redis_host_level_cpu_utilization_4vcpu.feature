...
Feature: Alarm Setup - Elasticache Errors
  ...
  Scenario: Create elasticache:alarm:health-redis_cpu_utilization:2020-04-01 with 4 vCPU based on CPUUtilization metric and check OK status
    ...
    When alarm "elasticache:alarm:health-redis_host_level_cpu_utilization:2020-04-01" is installed
      | alarmId    | ClusterId                         | Threshold | SNSTopicARN                       |
      | under_test | {{cache:before>PrimaryClusterId}} | 90        | {{cfn-output:SnsForAlarms>Topic}} |
