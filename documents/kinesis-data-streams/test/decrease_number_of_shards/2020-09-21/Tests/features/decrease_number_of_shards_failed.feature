...
Feature: SSM automation document Digito-DecreaseNumberOfKinesisDataStreamsShardsTest_2020-09-21
  ...
  Scenario: Execute SSM automation document Digito-DecreaseNumberOfKinesisDataStreamsShardsTest_2020-09-21 to test failure case
    ...
    And Wait for the SSM automation document "Digito-DecreaseNumberOfKinesisDataStreamsShardsTest_2020-09-21" execution is on step "AssertAlarmToBeRed" in status "TimedOut"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    # Add any step required to rectify the alarm here

    Then Wait for the SSM automation document "Digito-DecreaseNumberOfKinesisDataStreamsShardsTest_2020-09-21" execution is on step "AssertAlarmToBeGreen" in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And SSM automation document "Digito-DecreaseNumberOfKinesisDataStreamsShardsTest_2020-09-21" execution in status "TimedOut"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
