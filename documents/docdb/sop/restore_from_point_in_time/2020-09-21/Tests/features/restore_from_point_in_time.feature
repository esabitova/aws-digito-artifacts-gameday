...
Feature: SSM automation document to recover the database into a known good state.
  ...
  Scenario: Recover the database into a known good state using latest point-in-time
    ...
    And upload file "{{cache:DistributionPackageRelativePath}}" as "{{cache:DistributionPackageS3Key}}" S3 key to "ssm-docdb-test-resources" S3 bucket and save locations to "CloudWatchCanary" cache property
