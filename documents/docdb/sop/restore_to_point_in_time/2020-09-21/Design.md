...
1. `RestoreToDateTimeDynamoDBTableToPointInTime`
    ...
    * Explanation:
        ...
        * If `RestoreToDateTime` is a defined date value, then `RestoreDateTime` property will be used when calling the [restore_table_to_point_in_time] (https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_RestoreTableToPointInTime.html) method.
...
1. `RestoreToLatestDynamoDBTableToPointInTime`
    ...
    * Outputs:
        * `TargetTableArn`: The ARN of the target table recovered to the point in time
    * Explanation:
        * Restore to the latest time using the `UseLatestRestorableTime` property.[restore_table_to_point_in_time] (https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_RestoreTableToPointInTime.html) method.
...
1. `VerifyDynamoDBTableTargetStatusAfterRestoreTableToPointInTime`
    ...
    * Explanation:
        * Wait for the restored table to be in ACTIVE status after the restore to point in time operation is completed. [describe_table](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_DescribeTable.html) method
