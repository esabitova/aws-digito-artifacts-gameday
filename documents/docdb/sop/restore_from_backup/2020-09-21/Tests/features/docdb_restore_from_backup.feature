...
Feature: SSM automation document to recover the database into a known good state.
  ...
  Scenario: Recover the database into a known good state using latest snapshot
    ...
    And published "Digito-RestoreDocumentDBClusterFromBackupSOP_2020-09-21" SSM document
    ...
    And prepare replaced cluster for teardown
      | DBClusterIdentifier                              |
      | {{cfn-output:DocDbTemplate>DBClusterIdentifier}} |
    When SSM automation document "Digito-RestoreDocumentDBClusterFromBackupSOP_2020-09-21" executed
      | DBClusterIdentifier                              | AutomationAssumeRole                                                                              |
      | {{cfn-output:DocDbTemplate>DBClusterIdentifier}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoRestoreDocumentDBClusterFromBackupSOPAssumeRole}} |
    Then SSM automation document "Digito-RestoreDocumentDBClusterFromBackupSOP_2020-09-21" execution in status "Success"
      | ExecutionId                |
  ...
  Scenario: Recover the database into a known good state using specified snapshot identifier
    ...
    When SSM automation document "Digito-RestoreDocumentDBClusterFromBackupSOP_2020-09-21" executed
      ...
      | {{cache:before>SnapshotId}} | {{cfn-output:DocDbTemplate>DBClusterIdentifier}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoRestoreDocumentDBClusterFromBackupSOPAssumeRole}} |
    Then SSM automation document "Digito-RestoreDocumentDBClusterFromBackupSOP_2020-09-21" execution in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
    And wait for instances to be available for "120" seconds
