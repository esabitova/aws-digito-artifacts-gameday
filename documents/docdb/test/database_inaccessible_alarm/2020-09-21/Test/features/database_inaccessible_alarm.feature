...
Feature: SSM automation document to test database alarm.
  ...
  Scenario: Test database alarm
    ...
    Then Wait for the SSM automation document "Digito-ForceDocumentDBDatabaseToBeInaccessibleTest_2020-09-21" execution is on step "RemoveEmptySecurityGroup" in status "Success"
      | ExecutionId               |
      | {{cache:SsmExecutionId>1}}|
    And stop canary
      | CanaryName                                                         |
      | {{cfn-output:DocDbTemplate>DocumentDbConnectionAttemptCanaryName}} |
    Then SSM automation document "Digito-ForceDocumentDBDatabaseToBeInaccessibleTest_2020-09-21" execution in status "Success"
      | ExecutionId                |
      | {{cache:SsmExecutionId>1}} |
