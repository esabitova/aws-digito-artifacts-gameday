...
Feature: SSM automation document to create backup of EFS
  ...
  Scenario: Create AWS resources using CloudFormation template and execute SSM automation document to create backup of EFS
    ...
    And published "Digito-CreateEFSBackupSOP_2020-10-26" SSM document
    And SSM automation document "Digito-CreateEFSBackupSOP_2020-10-26" executed
      | FileSystemId                     | BackupVaultName                                  | BackupJobIamRoleArn                      | AutomationAssumeRole                                                           |
      | {{cfn-output:EFSTemplate>EFSID}} | {{cfn-output:EFSTemplate>BackupVaultSourceName}} | {{cfn-output:EFSTemplate>JobIAMRoleArn}} | {{cfn-output:AutomationAssumeRoleTemplate>DigitoCreateEFSBackupSOPAssumeRole}} |

    When SSM automation document "Digito-CreateEFSBackupSOP_2020-10-26" execution in status "Success"
      | ExecutionId                |
