...
def check_existence_of_objects_in_bucket(events, context):
    ...
    return {'NumberOfObjectsExistInRestoreBucket': number_of_objects_exist_in_restore_bucket,
...
def clean_bucket(events, context):
    ...
    for version in versions:
        ...
        print(
            f'The versioned object with Bucket={s3_bucket_to_restore_name}, Key={key}, VersionId={version_id} was deleted')
    ...
    for delete_marker in delete_markers:
        ...
        s3_client.delete_object(Bucket=s3_bucket_to_restore_name, Key=key, VersionId=version_id)
        print(
            f'The delete marker with Bucket={s3_bucket_to_restore_name}, Key={key}, VersionId={version_id} was deleted')
        number_of_deleted_objects += 1

    print(
        f'The number of deleted versioned objects and delete markers in restore bucket is {number_of_deleted_objects}')
...
def restore_from_backup(events, context):
    ...
    list_objects_v2_response = s3_client.list_objects_v2(Bucket=s3_backup_bucket_name)
    copied_count = 0
    print(
        f'Starting to copy files from the {s3_backup_bucket_name} bucket to the {s3_bucket_to_restore_name} bucket...')
    for content in list_objects_v2_response["Contents"]:
