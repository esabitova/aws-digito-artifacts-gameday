...
class TestPublishDocuments(unittest.TestCase):
    ...
    def test_alarm_document_variable_parsing_valid(self):
      ...
      AlarmDescription:
        Fn::Join: [ '', [ 'Alarm by Digito that reports CPU utilization is over ', ${Thres hold}, '%' ] ]
      AlarmName: ${AlarmName}
    ...
    """).get_variables() == set([])
        ...
        assert AlarmDocumentParser("""
      AlarmDescription:
        Fn::Join: [ '', [ 'Alarm by Digito that reports when CPU utilization is over ', ${Threshold}, '%' ] ]
      AlarmName: Fn::Join: [ '', [${AlarmName} , 'with ', ${Threshold}] ]
    """).get_variables() == set(["Threshold", "AlarmName"])

        # Same Line
        assert AlarmDocumentParser("AlarmName: Fn::Join: ['', [${AlarmName2}, 'with ', ${Threshold2}] ]").get_variables(
        ) == set(["Threshold2", "AlarmName2"])

    def test_alarm_document_variable_parsing_invalid(self):
        # Invalid expressions
        assert AlarmDocumentParser(
            """AlarmName: Fn::Join: [ '', [${}, 'with ', {Threshold2}] ${]""").get_variables() == set([])
    ...
    def test_alarm_document_replace_variables(self):
        under_test = AlarmDocumentParser(
            "AlarmName: Fn::Join: [ '', [${AlarmName} , 'with ', ${Threshold}, ${Threshold}] ]")
        assert under_test\
