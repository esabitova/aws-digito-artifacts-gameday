...
class ResourcePool:
            ...
            for i in range(pool_size):
                ...
                try:
                    ...
                    # give a try to update resource so that testing session is not terminated.
                        ...
                        if resource.type == ResourceModel.ResourceType.ASSUME_ROLE.name:
    ...
    def _filter_resource_by_index_and_type(self, resources: [], index: int,
                                           resource_type: ResourceModel.ResourceType) -> ResourceModel:
        """
    ...
    def _get_resource_pool_size(self, cfn_template_name: str, resource_type: ResourceModel.ResourceType) -> int:
        """
        ...
        """
        pool_size = 1
        if resource_type != ResourceModel.ResourceType.ON_DEMAND and \
                resource_type != ResourceModel.ResourceType.DEDICATED:
            return pool_size
    ...
    def _update_resource_stack(self, resource: ResourceModel, resource_type: ResourceModel.ResourceType,
        ...
        if resource_type == ResourceModel.ResourceType.ON_DEMAND or \
                resource_type == ResourceModel.ResourceType.DEDICATED:
            resource.status = ResourceModel.Status.LEASED.name
        else:
    ...
    def _get_cfn_template_path_by_type(self, cfn_template_path: str, resource_type: ResourceModel.ResourceType):
        ...
        return config.ssm_assume_role_cfn_s3_path if \
            resource_type == ResourceModel.ResourceType.ASSUME_ROLE else cfn_template_path
    ...
    def _get_stack_name(self, cfn_template_path: str, index: int, resource_type: ResourceModel.ResourceType):
        """
    ...
    def _get_cfn_template_file_name(self, cfn_template_path: str, resource_type: ResourceModel.ResourceType):
        """
        ...
        """
        if resource_type != ResourceModel.ResourceType.ASSUME_ROLE and not os.path.isfile(cfn_template_path):
