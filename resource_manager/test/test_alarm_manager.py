  ...
  VolumeWriteBytes:
    ...
    Properties:
      AlarmActions:
        - Ref: SNSTopicARN
      AlarmDescription:
        Fn::Join: [ '', [ 'Alarm by Digito that reports when volume write throughput is anomalous with band width ', 1 ] ]
...
class TestAlarmManager(unittest.TestCase):
  ...
  def test_deploy_alarm_failure(self):
    ...
    self.cfn_helper_mock.describe_cf_stack_events.return_value = [
      ...
      {"LogicalResourceId":"res2", "ResourceStatus":"CREATE_FAILED",  "ResourceStatusReason":"Internal_Failure_Reason"}
